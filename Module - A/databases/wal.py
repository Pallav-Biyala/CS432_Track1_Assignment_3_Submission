    ...
    def recover(self, db_manager):
        ...
        with open(self.filename, "r") as f:
            for line in f:
                ...
                if line.startswith("Begin"):
                    ...
                    old_value = None if old_value == "None" else ast.literal_eval(old_value)
                    new_value = None if new_value == "None" else ast.literal_eval(new_value)
        
                    op = Operations(table, op_type, key, old_value, new_value)
        ...
        for tid, ops in active.items():
            if tid not in committed:
                print(f"Undoing incomplete transaction {tid}")
